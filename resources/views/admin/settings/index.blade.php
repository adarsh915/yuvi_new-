            ...
            <form action="{{ route('admin.settings.update') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card p-0 radius-12 bg-base mb-4">
                    ...
                    <div class="card-body p-24">
                        <div class="row gy-4">
                            <div class="col-md-6">
                                <label class="form-label fw-semibold text-sm mb-8">Header Logo</label>
                                <div class="mb-12">
                                    @php $hLogo = $settings->where('key', 'header_logo')->first()->value ?? ''; @endphp
                                    <img src="{{ asset(str_contains($hLogo, 'settings/') ? 'storage/' . $hLogo : $hLogo) }}"
                                        alt="Header Logo"
                                        style="max-height: 80px; background: #eee; padding: 10px; border-radius: 8px;">
                                </div>
                                <input type="file" name="files[header_logo]" class="form-control">
                                <p class="text-xs text-secondary-light mt-8">Recommended size: 300x120px (PNG with
                                    transparency)</p>
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-semibold text-sm mb-8">Footer Logo</label>
                                <div class="mb-12">
