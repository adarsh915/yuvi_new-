...
@section('content')
  ...
  <div class="page-body">
    ...
    <aside class="filter-sidebar">
      ...
      <hr class="fs-divider">
      <div class="fs-note"><strong>Privacy First.</strong><br>All stories are shared with full informed consent. Names may be changed to protect privacy.</div>
    </aside>
    ...
    <div class="story-content-main">
      ...
      <div class="mobile-filter-panel" id="mobileFilterPanel">
        ...
        @foreach($categories as $cat)
          <button class="filter-btn" data-filter="{{ strtolower(str_replace(' ', '-', $cat)) }}"><span class="dot"></span>{{ $cat }}</button>
        @endforeach
      </div>
      ...
      <div class="story-page-grid" id="storyPageGrid">
        ...
        <article class="story-page-card reveal" data-category="{{ strtolower(str_replace(' ', '-', $story->treatment_type)) }}">
          <div class="story-page-video-wrap">
            <iframe src="{{ $story->video_url }}" loading="lazy" allow="autoplay; encrypted-media" allowfullscreen title="{{ $story->title }}"></iframe>
  ...
  <style>
    ...
    @media (max-width: 1100px) {
        ...
        .story-page-grid { grid-template-columns: repeat(2, 1fr); }
    }
